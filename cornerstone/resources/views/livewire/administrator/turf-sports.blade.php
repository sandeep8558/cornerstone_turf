...
new class extends Component {
    ...
    public function addSport() {
        $this->reset(['turf_id', 'name', 'is_active', 'editingSport']);
        $this->dispatch('open-modal', 'sport-modal');
    }

    public function editSport(TurfSport $sport) {
        $this->editingSport = $sport;
        $this->turf_id = $sport->turf_id;
    ...
    public function saveSport() {
        ...
        if ($this->editingSport) {
            $this->editingSport->update($validated);
            session()->flash('message', 'Sport updated successfully.');
        } else {
            TurfSport::create($validated);
            session()->flash('message', 'Sport added successfully.');
        }

        $this->dispatch('close-modal', 'sport-modal');
    }

    public function deleteSport(TurfSport $sport) {
        $sport->delete();
...
<div class="card border-0 shadow-sm rounded-4 overflow-hidden text-dark">
    ...
    <div class="card-header bg-white border-bottom border-light px-4 py-3">
        <div class="row align-items-center g-3">
            <div class="col-md-4">
                <h2 class="h6 fw-semibold text-dark mb-0">Turf Sports ({{ $sports->total() }})</h2>
            </div>
            <div class="col-md-5">
            ...
            <div class="col-md-3 text-md-end">
                <button wire:click="addSport" class="btn btn-primary btn-sm d-inline-flex align-items-center gap-2 rounded-2">
    ...
    <div class="table-responsive">
        <table class="table table-hover align-middle mb-0 small text-dark">
            <thead class="table-light text-muted text-uppercase" style="font-size: 0.65rem; letter-spacing: 0.05em;">
                <tr>
                    <th class="px-4 py-3 text-start fw-semibold">Sport Name</th>
            ...
            <tbody class="divide-y">
                @forelse($sports as $s)
                    <tr wire:key="sport-{{ $s->id }}">
                        <td class="px-4 py-3 fw-bold">{{ $s->name }}</td>
                        <td class="px-4 py-3">{{ $s->turf->name }}</td>
                        <td class="px-4 py-3">
                            <span class="badge rounded-pill bg-{{ $s->is_active === 'Yes' ? 'success' : 'info' }}-subtle text-{{ $s->is_active === 'Yes' ? 'success' : 'info' }} border border-{{ $s->is_active === 'Yes' ? 'success' : 'info' }}-subtle fw-medium">
                                {{ $s->is_active }}
                            </span>
                        </td>
                        <td class="px-4 py-3 text-end text-nowrap">
                            <div class="d-flex align-items-center justify-content-end gap-2">
                                <button wire:click="editSport({{ $s->id }})" class="btn btn-outline-primary btn-sm rounded-pill px-3 py-1" style="font-size: 0.75rem;">Edit</button>
                                <button onclick="confirm('Are you sure?') || event.stopImmediatePropagation()" wire:click="deleteSport({{ $s->id }})" class="btn btn-outline-danger btn-sm rounded-pill px-3 py-1" style="font-size: 0.75rem;">Remove</button>
                            </div>
                        </td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="4" class="px-4 py-5 text-center text-muted">
                            <p class="small mb-0">No sports found.</p>
                        </td>
                    </tr>
                @endforelse
            </tbody>
    ...
    <!-- Sport Modal -->
    <div class="modal fade" id="sportModal" tabindex="-1" aria-hidden="true" wire:ignore.self>
        <div class="modal-dialog modal-dialog-centered">
            <form wire:submit="saveSport" class="modal-content border-0 shadow-lg rounded-4 overflow-hidden">
                <div class="modal-header bg-light border-bottom px-4 py-3">
                    <h5 class="modal-title h6 fw-bold mb-0">{{ $editingSport ? 'Edit Sport' : 'Add Turf Sport' }}</h5>
                ...
                <div class="modal-body p-4 text-dark">
                    ...
                    <div class="mb-3">
                        <label class="form-label small fw-semibold text-muted">Sport Name</label>
                        <input wire:model="name" type="text" class="form-control rounded-3" placeholder="e.g. Football, Cricket, Tennis...">
    ...
    <script>
        $wire.on('open-modal', () => {
            const modal = bootstrap.Modal.getOrCreateInstance(document.getElementById('sportModal'));
            modal.show();
        });
        $wire.on('close-modal', () => {
            const modal = bootstrap.Modal.getInstance(document.getElementById('sportModal'));
            if (modal) modal.hide();
        });
    </script>
    @endscript
